...
def listen_command():
    ...
    try:
        command_heard = r.recognize_sphinx(audio).lower()
        print("You said: " + command_heard)
        return command_heard
...
def generate_new_people(num_people):
    habits = ["eating", "breathing", "sleeping", "working", "walking", "shopping", "studying", "cooking", "cleaning", "exercising"]
    locations = ["Bucuresti"] * num_people
    new_people = [Person(i, f"Person {len(vectpers) + i + 1}", "healthy", "now", random.choice(habits), random.choice(locations)) for i in range(num_people)]
    for person in new_people:
        person.disease = reasoning1.determineDisease()
    return new_people
...
if __name__ == '__main__':
    ...
    while True:
        ...
        announcement = f"At {current_time}, in Bucuresti with a temperature of {envTemperature} degrees and a noise level of {noiseLevel} dB, we have the following patients:"
        print(announcement)
        announce(announcement)

        for person in vectpers:
            report = f"{person.name} has {person.disease.name} with {person.disease.severity} severity"
            print(report)
            announce(report)

        command = listen_command()
        if "more" in command:
            new_people = generate_new_people(10)
            vectpers.extend(new_people)
            for person in new_people:
                report = f"{person.name} has {person.disease.name} with {person.disease.severity} severity"
                print(report)
                announce(report)
